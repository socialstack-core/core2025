 ...
 */
const Search: React.FC<SearchProps > = ({ label, id, className, xs, sm, md, lg, xl, wrapperClass, noWrapper, ...rest }) => {

	function renderField() {
		const _id = id || useId();

		// filter out only valid search attributes
		const searchProps: React.InputHTMLAttributes<HTMLInputElement> =
			Object.fromEntries(
		...
		var fieldSearch = `${COMPONENT_PREFIX}-search`;
		var componentClasses = [fieldSearch];
		componentClasses = componentClasses.concat(getSizeClasses(fieldSearch, { xs: Boolean(xs), sm: Boolean(sm), md: Boolean(md), lg: Boolean(lg), xl: Boolean(xl) }));
		...
		return <>
			...
			<input type="search" id={_id} className={componentClasses.join(' ')} {...searchProps} />
		</>;
	}
