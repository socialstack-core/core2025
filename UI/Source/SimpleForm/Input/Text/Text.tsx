...
/**
 * The Text React component.
 * @param props Text props.
 */
const Text: React.FC<TextProps > = ({ label, id, className, xs, sm, md, lg, xl, wrapperClass, noWrapper, ...rest }) => {

	function renderField() {
		const _id = id || useId();

		// filter out only valid text attributes
		const textProps: React.InputHTMLAttributes<HTMLInputElement> =
			Object.fromEntries(
		...
		var fieldText = `${COMPONENT_PREFIX}-text`;
		var componentClasses = [fieldText];
		componentClasses = componentClasses.concat(getSizeClasses(fieldText, { xs: Boolean(xs), sm: Boolean(sm), md: Boolean(md), lg: Boolean(lg), xl: Boolean(xl) }));
		...
		return <>
			...
			<input type="text" id={_id} className={componentClasses.join(' ')} {...textProps} />
		</>;
	}
