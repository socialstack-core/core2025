...
/**
 * The Range React component.
 * @param props Range props.
 */
const Range: React.FC<RangeProps > = ({ label, id, className, xs, sm, md, lg, xl, wrapperClass, noWrapper, ...rest }) => {

	function renderField() {
		const _id = id || useId();

		// filter out only valid range attributes
		...
		var fieldRange = `${COMPONENT_PREFIX}-range`;
		var componentClasses = [fieldRange];
		componentClasses = componentClasses.concat(getSizeClasses(fieldRange, { xs: Boolean(xs), sm: Boolean(sm), md: Boolean(md), lg: Boolean(lg), xl: Boolean(xl) }));
		...
		return <>
			...
			<input type="range" id={_id} className={componentClasses.join(' ')} {...rangeProps} />
		</>;
	}
