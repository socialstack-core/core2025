...
public partial class AutoController<T,ID>
{
	...
	/// <returns></returns>
	[HttpGet("list.csv")]
	public virtual async ValueTask<FileContent?> ListCSV(Context context, HttpResponse response, [FromQuery] string includes = null)
	{
		return await ListCSV(context, response, null, includes);
	}
	...
	public virtual async ValueTask<FileContent?> ListCSV(Context context, HttpResponse response, [FromBody] JObject filters, [FromQuery] string includes = null)
	{
		...
		return new FileContent(ms.GetBuffer(), "text/csv", typeof(T).Name + ".csv");
	}
